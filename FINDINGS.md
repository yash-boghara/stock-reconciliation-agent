...
*Historical: the figures in this section were measured on the dataset as it
stood before the fix, and that dataset no longer exists. Everything after it
is regenerated by the harness.*
...
Ceiling moved **61.3% → 79.5%** [77.6–81.2], and shrinkage became
predictable. The current figure is fitted on 60 seeds and measured on 40 held
out — see [docs/evaluation.md](docs/evaluation.md).
...
| Structured features, Bayes-optimal ceiling | 79.5% | 77.6 – 81.2 |
| Heuristic control, structured features only | **76.0%** | 74.7 – 77.2 |

A deterministic function landed within three and a half points of
information-theoretic optimal, with overlapping intervals. The context separating these causes — is the line short-dated, is it
